...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<AgencyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/agencies` : null),
    () => getAgencies({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Agency',
      path: isTenantUser ? `/agencies/view/${data?.[0]?.id}` : '/agencies',
      entity: 'agency',
